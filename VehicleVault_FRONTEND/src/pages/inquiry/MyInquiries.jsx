            ...
            inquiries.map((iq) => (
              <div key={iq._id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  ...
                  {/* VEHICLE PREVIEW */}
                  <div className="md:w-1/3 bg-slate-100 p-6 flex flex-col justify-center border-r border-slate-200">
                    {iq.vehicle_id?.images?.[0] && (
                      <img 
                        src={iq.vehicle_id.images[0]} 
                        alt="car" 
                  ...
                  <div className="md:w-2/3 p-6">
                    <div className="flex justify-between items-start mb-4">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                        iq.status === "resolved" ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"
                      }`}>
                        {iq.status}
                      </span>
