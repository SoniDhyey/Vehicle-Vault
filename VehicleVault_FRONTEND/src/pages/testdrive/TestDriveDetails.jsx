          ...
          requests.map((req) => (
            <div key={req._id} className="bg-white p-6 rounded-3xl border shadow-sm flex justify-between items-center">
              ...
              <div className="flex gap-2">
                {req.status === "Pending" ? (
                  <>
                    <button onClick={() => handleStatusUpdate(req._id, "Approved")} className="bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-emerald-600 transition-colors">Approve</button>
                    <button onClick={() => handleStatusUpdate(req._id, "Cancelled")} className="bg-red-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-red-600 transition-colors">Reject</button>
                  </>
                ) : (
                  <span className={`px-4 py-2 rounded-lg font-bold uppercase text-xs ${req.status === "Approved" ? "text-emerald-600 bg-emerald-50" : "text-red-600 bg-red-50"}`}>{req.status}</span>
                )}
              </div>
            </div>
          ))
        ) : (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed"><p className="text-slate-400 font-medium">No test drive requests found.</p></div>
        )}
      </div>
    </div>
  );
